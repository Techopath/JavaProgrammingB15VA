...
public class EmployeesArray {
    public static void main(String[] args) {
        ...
        //task1. Print all job titles - using substring ,indexOf

//using for each
        for (String emp:employees){
            //System.out.println(emp);
            String jobs = emp.substring(emp.indexOf("[")+1, emp.indexOf("]"));
...
//           String jobTitles= employees[i].substring(employees[i].indexOf("[")+1,employees[i].indexOf("]"));
//            System.out.println(jobTitles);
//        }
        //task 2. Print first name -last name jobtitle
        /*
        philipa-salhouse electrical engineer

         */
        for (int i = 0; i < employees.length; i++) {
            String[] profileInfo = employees[i].split(" ");
            System.out.println(profileInfo[0] +"-"+profileInfo[1]+" ");
        }
        //
        ...
        for (String emp : employees){
            ...
            String jobTitles = emp.substring(emp.indexOf("[")+1, emp.indexOf("]"));
            System.out.println(first+"-"+last+" "+jobTitles);
        }
        //
       //look for those 2 people in the array and assert below data

        /* Tracy Bartle [Geological Engineer]
        Skipper Abrahamson [Financial Assistance]
        */
        String pass = "";
        for (String emp : employees){
            String [] empArr= emp.split(" ");
            ...
            if (first.equals("Tracy") && last.equals("Bartle") ){
              if(jobTitles.equals("Geological Engineer")){
                    System.out.println("pass Tracy");
                }else {
                    System.out.println("Tracy failed");
                }
            }
           if (first.equals("Skipper") && last.equals("Abrahamsson")){
                if(jobTitles.equals("Financial Assistant")){
                    System.out.println("pass Skipper");
                }else {
                    System.out.println("Skipper failed");
                }
            }
            //todo: add condition to check Skipper's data
        ...
        for (int i = 0; i < employees.length; i++) {
            String fnamelnamejobtitle = employees[i];
            //System.out.println("fnamelnamejobtitle: "+fnamelnamejobtitle);
            String [] initialsArray = fnamelnamejobtitle.split(" ");//splited each employees profile separately

            //System.out.println(Arrays.toString(initialsArray));

            String name = initialsArray[0];//prints out all the names
            String l_name = initialsArray[1];//prints out all the last names
            String jobs = fnamelnamejobtitle.substring(fnamelnamejobtitle.indexOf("[")+1,fnamelnamejobtitle.indexOf("]"));//extracts job titles without []
...
//            System.out.println(jobs);
            ...
            String initial2 =l_name.substring(0,1);
            System.out.println(initial1+initial2+"-->"+jobs);
        }
